...
def main():
    ...
    parser.add_argument('--output_name', '-o', type=str, default=None,
                        help='Output filename (without extension). Default: attention_features_{filter_mode}_n{samples}')
    parser.add_argument('--max_query_tokens', type=int, default=None,
                        help='Maximum tokens per query (default: None). Queries longer than this will be truncated.')
    ...
    if args.compare:
        ...
        baseline_features, all_labels, all_query_ids, all_doc_ids, docs_per_query = extract_with_batching(
            extractor, data, args.batch_size, args.max_doc_tokens, args.max_query_tokens,
            input_format, qrels, args.relevance_threshold, args.max_docs, desc="Baseline"
        )
    ...
    if args.compare:
        ...
        all_features, _, _, _, _ = extract_with_batching(
            extractor, data, args.batch_size, args.max_doc_tokens, args.max_query_tokens,
            input_format, qrels, args.relevance_threshold, args.max_docs, desc="Filtered"
        )
    else:
